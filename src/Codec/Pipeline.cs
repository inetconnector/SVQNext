...
namespace SVQNext.Codec
{
    ...
    {
        public static Mux EncodeToMux(IReadOnlyList<float[,,]> rgb, string quality, string searchMode, int bs, int search, int QMotion, int gop, bool useB, bool filters, bool useHdr, int bitdepth, string colorspace, bool scalable, int targetKbps)
        {
            ...
            int T = rgb.Count;
            int H = rgb[0].GetLength(0);
            int W = rgb[0].GetLength(1);
            // Colorspace
            ...
            int sampleFrames = Math.Max(1, Math.Min(8, YForCodebook.Length-1));
            ...
            var chunks = new List<Chunk>();
            {
                ...
                int codeK = code.GetLength(0);
                int D = code.GetLength(1);
                bw.Write(codeK); bw.Write(D);
                for (int k=0;k<codeK;k++) for (int j=0;j<D;j++) bw.Write(code[k,j]);
                for (int k=0;k<codeK;k++) bw.Write(mu[k]);
        ...
        public static (float[][,,] Rgb, int T, int H, int W) DecodeFromMux(Mux mux)
        {
            ...
            string colorspace = v.Meta.TryGetValue("colorspace", out var cs) ? cs : "bt709";
            bool useHdr = v.Meta.TryGetValue("hdr", out var hdrFlag) && hdrFlag == "1";
            int bitDepth = v.Meta.TryGetValue("bitdepth", out var depthStr) && int.TryParse(depthStr, out var parsedDepth) ? parsedDepth : 10;

            if (v.Chunks.Count == 0)
            {
                throw new InvalidDataException("video track has no chunks");
            }
            ...
            using (var cfgBr = new BinaryReader(cfgMs))
            {
                ...
                int cfgK = cfgBr.ReadInt32();
                int D = cfgBr.ReadInt32();
                codebook = new float[cfgK, D];
                for (int k=0;k<cfgK;k++)
                    for (int j=0;j<D;j++)
                        codebook[k,j] = cfgBr.ReadSingle();
                mu = new float[cfgK];
                for (int k=0;k<cfgK;k++) mu[k] = cfgBr.ReadSingle();
            }

            int frameCount = v.Chunks.Count - 1;
            ...
            if (v.Meta.TryGetValue("loop", out var loopStr))
            {
                seq.Loop = loopStr != "0";
            }
            if (v.Meta.TryGetValue("useb", out var useBStr))
            {
                seq.UseB = useBStr == "1";
            }
            if (v.Meta.TryGetValue("gop", out var gopStr) && int.TryParse(gopStr, out var gopVal))
            {
                seq.GOP = gopVal;
            }
            ...
            for (int idx=0; idx<frameCount; idx++)
            {
                ...
                for (int i=0;i<nIdx;i++) frame.Idx[i]=br.ReadUInt16();
                int nDc=br.ReadInt32();
                frame.DCq=new short[nDc];
                for (int i=0;i<nDc;i++) frame.DCq[i]=br.ReadInt16();
                int H2=br.ReadInt32();
                int W2=br.ReadInt32();
                int cbLen=br.ReadInt32();
                var cbQ=br.ReadBytes(cbLen);
                int crLen=br.ReadInt32();
            ...
            bool applyFilters = !v.Meta.TryGetValue("filters", out var filterStr) || filterStr != "0";
            var Yrec = Decoder.DecodeY(seq, codebook, mu, bs, filters: applyFilters);

            if (useHdr)
            {
                for (int t=0; t<Yrec.Length; t++)
                {
                    var Yplane = Yrec[t];
